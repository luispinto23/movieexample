...
// NOTE: the controller logic is different from the repository one.
// The repository provides an interface to get all ratings for a record
// and the controller implements the aggregation logic for them.
...
func (c *Controller) StartIngestion(ctx context.Context) error {
	...
	for e := range ch {
		err := c.PutRating(ctx, e.RecordID, e.RecordType, &model.Rating{
			UserID: e.UserID,
			Value:  e.Value,
		})
		if err != nil {
